...
const STORAGE_KEY = 'cheche:notification-settings';
...
function readStoredSettings(): { settings: NotifSetting[]; ready: boolean } {
  if (typeof window === 'undefined') {
    return { settings: INITIAL_SETTINGS, ready: false };
  }

  try {
    const raw = window.localStorage.getItem(STORAGE_KEY);
    if (!raw) {
      return { settings: INITIAL_SETTINGS, ready: true };
    }

    const parsed = JSON.parse(raw) as Array<{ id: string; on: boolean }>;
    if (!Array.isArray(parsed)) {
      return { settings: INITIAL_SETTINGS, ready: true };
    }

    const savedMap = new Map(parsed.map((item) => [item.id, item.on]));
    return {
      settings: INITIAL_SETTINGS.map((setting) =>
        savedMap.has(setting.id) ? { ...setting, on: Boolean(savedMap.get(setting.id)) } : setting
      ),
      ready: true,
    };
  } catch {
...
export default function NotificationsPage() {
  ...
  const [{ settings: initialSettings, ready }] = useState(() => readStoredSettings());
  const [settings, setSettings] = useState<NotifSetting[]>(initialSettings);

  const toggle = (id: string) =>
    setSettings((prev) => prev.map((setting) => (setting.id === id ? { ...setting, on: !setting.on } : setting)));
  ...
  const handleSave = () => {
    ...
    try {
      window.localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify(settings.map(({ id, on }) => ({ id, on })))
      );
    } catch {}
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--bg-page)' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 0 120px' }}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.4 }}>{setting.desc}</div>
                </div>
              </div>
              <Toggle on={setting.on} onChange={() => toggle(setting.id)} />
            </div>
          ))}
        </div>
        ...
        >
          <div style={{ fontSize: 12, color: 'var(--brand-text)', fontWeight: 600, marginBottom: 4 }}>
            알림 발송 시간
          </div>
          <div style={{ fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
            D-3와 당일 알림은 오전 9시에 발송됩니다.
            <br />
            기간 초과 알림은 마감 다음 날 오전 9시에 발송됩니다.
          </div>
        </div>
      </div>
